...
public class P00_Receipt {
	// 전역변수로 선언
	static String[] k27_itemname = { "해피홈 SAFE365 핸드크림", "(G)LH원형유리화병 100cm", "상투과자E", "더블링 나일론 깡 롤링", "해피홈 SAFE 핸드워시",
			...
			"흑원당 말차 밀크티", "1+ 등급란 10개입 특", "동원양반볶음김치기획", "오리온 초코파이 468g*12", "바나나우유 120ml*4", "수향미 골든 퀸 10Kg+10KG",
			"ABC초코 187g" }; // 상품 30개 이상을 문자열 배열에 추가

	static int[] k27_price = { 97500, 96900, 95980, 93300, 95500, 1113360, 196610, 92750, 10800, 4980, 2480, 97920,
			94980, 4980, 7130, 3980, 4480, 4980, 3210, 6950, 2980, 1990, 1980, 3980, 4480, 19480, 7980, 3620, 3720,
			27980, 4000 };
	// 각각의 가격을 정수형 배열에 저장
	static int[] k27_num = { 10, 10, 10, 10, 10, 10, 10, 10, 20, 1, 1, 10, 10, 1, 1, 1, 1, 1, 1, 1, 2, 1, 1, 1, 1, 50,
			1, 1, 1, 30, 1 };
	// 각각의 수량을 정수형 배열에 저장
	static boolean[] k27_taxfree = { true, false, false, false, false, false, true, false, true, true, false, false,
	...
	static int k27_no_tax = 0; // 면세물품가격에 대한 변수 선언과 초기화
	static int k27_totalsum = 0; // 총 가격 변수 선언과 초기화
	static int k27_point = 5473; // 직전 포인트 변수 선언과 대입

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		// 영수증 출력 3 을 쪼개기 메인을 심플하게
		//
		int k27_item_count = k27_itemname.length;
		k27_titel_print(); // 영수증 윗부분 출력 함수
		k27_time_stemp(); // 날짜 부분 출력 함수
		k27_header_print(); // 헤더(상품명,단가,수량,금액) 출력

		for (int k27_i = 0; k27_i < k27_item_count; k27_i++) { // 아이템 개수만큼 for문 돌리기
			k27_item_print(k27_i); // 항목갯수만큼 항목인쇄
		}
		k27_total_print(); // 합계 세액등을 인쇄
	}
	...
	public static void k27_time_stemp() {
	...
	public static void k27_header_print() {
		k27_line_print();
		System.out.printf("%7s%13s%4s%8s\n", "상 품 명", "단 가", "수량", "금 액");
		k27_line_print();
	}

	public static void k27_item_print(int k27_i) {
		...
		System.out.printf("%s%10s%3d%11s\n", k27_subStrByte(k27_itemname[k27_i], 15), k27_df.format(k27_price[k27_i]),
				k27_num[k27_i], k27_df.format(k27_price[k27_i] * k27_num[k27_i]));
	...
	public static void k27_total_print() {
		DecimalFormat k27_df = new DecimalFormat("###,###,###,###,###"); // 세자리마다 쉼표 찍기
		Calendar k27_calt = Calendar.getInstance(); // 오늘 날짜 가져오기
